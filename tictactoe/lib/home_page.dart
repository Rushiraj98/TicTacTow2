...
class _HomePageState extends State<HomePage> {
  ...
  void playGame(GameButton gb) {
    setState(() {
      ...
      if (winner == -1) {
        if (buttonsList.every((p) => p.text != "")) {   //Game tied check
          showDialog(
              context: context,
              builder: (_) => new CustomDialog("Game Tied",
                  "Press the reset button to start again.", resetGame));
        } else {
          activePlayer == 2 ? autoPlay() : null;        // calling auto play if not tied
        }
      }
    });
  }

  void autoPlay() {                                     //auto play
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
        appBar: new AppBar(
          ...
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.only(bottomLeft: Radius.circular(12.0), bottomRight: Radius.circular(12.0),)),
        ),
        body: new Column(
